...
import { ForecastEntryBatchError, saveForecastEntryBatch } from "@/services/forecastEntryBatchService";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await saveForecastEntryBatch({
      ...payload,
      createdBy: auth.user.displayName || auth.user.email
    });
    return NextResponse.json(result);
  } catch (error) {
    if (error instanceof ForecastEntryBatchError) {
    ...
    return NextResponse.json(
      {
        error: "Unable to save Forecast Entry batch",
