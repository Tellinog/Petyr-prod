...
function buildIncludedSignals(candidate: PetyrAiForecastCandidate): PetyrAiForecastIncludedSignal[] {
  ...
  if (candidate.plannedCampaignsValue > 0) signals.push(includedSignal({ code: "planned_campaigns", label: "Planned campaigns", role: "planned_future_floor", value: candidate.plannedCampaignsValue, reason: "Valid future planning-like, Setup, Recruiting or Running campaign value is used as the baseline floor for this month only." }));
...
function buildExcludedSignals(candidate: PetyrAiForecastCandidate): PetyrAiForecastRowExplainability["excludedSignals"] {
  ...
  if (candidate.plannedCampaignsValue <= 0) excluded.push({ code: "planned_campaigns", label: "Planned campaigns", role: "planned_future_floor", value: candidate.plannedCampaignsValue, reason: "No valid future planning-like, Setup, Recruiting or Running campaign value exists for this target month." });
